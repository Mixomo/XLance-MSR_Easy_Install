...
class Attend(nn.Module):
    ...
    def flash_attn(self, q, k, v):
        ...
        backends = []
        if config.enable_flash: backends.append(torch.nn.attention.SDPBackend.FLASH_ATTENTION)
        if config.enable_mem_efficient: backends.append(torch.nn.attention.SDPBackend.EFFICIENT_ATTENTION)
        if config.enable_math: backends.append(torch.nn.attention.SDPBackend.MATH)
